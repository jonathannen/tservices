...
- **Abort signal** -- propagates cancellation to any code that accepts an
  `AbortSignal`, so in-flight work can react to shutdown without polling.
- **Deferred promises** -- allows out-of-band work (e.g. background flushes)
...
- **Ordered destroy callbacks** -- registered callbacks run in order during
  ...
  rest.
- **Outcome** -- tracks whether shutdown completed cleanly or encountered
  errors.
